...
class BitLocker:
    ...
    @staticmethod
    def get_partition_icon_name(
        drive_dict: dict, drive_key: str, partition_key: str
    ) -> str:
    ...
    def mount_bitlocker_image_with_dislocker(
        self, partition_dev_node: str, password: str
    ) -> tuple[dict, str]:
        ...
        try:
            ...
            msg = _(
                f"Error attemping to run dislocker to decrypt the partition {partition_dev_node}.\n"
        ...
        print(
            f"Dislocker was able to mount the encrypted disk successfully at {unencrypted_partition}!"
        )
    ...
    def _parse_new_partition(filename: str):
        ...
        process, flat_command_string, fail_description = Utility.run(
            f"blkid {filename}", blkid_cmd_list, use_c_locale=True
        )
    ...
    def update_drive_state(
        drive_state: dict, partition: str, unencrypted_partition_info: dict
    ):
